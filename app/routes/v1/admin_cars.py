...
async def create_car(car_in: CarCreate):
    ...
    return ResponseModel(data=CarResponse.from_model(car), message="Car created successfully")
...
async def update_car(car_id: str, car_in: CarUpdate):
    ...
    return ResponseModel(data=CarResponse.from_model(car), message="Car updated successfully")
...
async def update_car_status(car_id: str, status_in: CarStatusUpdate):
    ...
    return ResponseModel(data=CarResponse.from_model(car), message="Car status updated successfully")

@router.delete("/{car_id}", response_model=ResponseModel[None])
